...
BASE_OUT="$OUT_DIR/2026-GAUTENGTREASURY-BIOMETRIC-AUTHENTICATION-proposal-final-base.docx"
OUT_FILE="$OUT_DIR/2026-GAUTENGTREASURY-BIOMETRIC-AUTHENTICATION-proposal-final.docx"
TMP_MD="$(mktemp)"
...

{
  ...
  printf -- '| Document Status | Submission copy |\n'
...
if [[ -x "$POLISH_SCRIPT" ]]; then
  python3 "$POLISH_SCRIPT" "$BASE_OUT" "$OUT_FILE" final
else
  cp "$BASE_OUT" "$OUT_FILE"
fi

echo "Final Word document created:"
echo "  $OUT_FILE"
